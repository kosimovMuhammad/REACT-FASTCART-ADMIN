...
import { addColor, updateColor, clearAddError, clearEditError } from '@/store/colorsSlice';

interface ColorModalProps {
  onClose: () => void;
  onCreated?: () => void;
  editItem?: { id: number; colorName: string };
}

export default function ColorModal({ onClose, onCreated, editItem }: ColorModalProps) {
  const dispatch = useDispatch<AppDispatch>();
  const { addLoading, addError, editLoading, editError } = useSelector((s: RootState) => s.colors);

  const isEdit = !!editItem;
  const [colorName, setColorName] = useState(editItem?.colorName ?? '');
  const [hex, setHex] = useState('#3b82f6');

  const loading = isEdit ? editLoading : addLoading;
  ...
  const handleSubmit = async () => {
    ...
    let result;
    if (isEdit) {
      result = await dispatch(updateColor({ id: editItem!.id, colorName: colorName.trim() }));
      if (updateColor.fulfilled.match(result)) {
        onCreated?.();
        onClose();
      }
    } else {
      result = await dispatch(addColor(colorName.trim()));
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-100 dark:border-zinc-800">
          <h3 className="font-semibold text-zinc-900 dark:text-white flex items-center gap-2">
            {isEdit
              ? <><Pencil size={18} className="text-indigo-500" /> Edit color</>
              : <><Plus size={18} className="text-indigo-500" /> New color</>
            }
          </h3>
          <button
        ...
        <div className="flex items-center justify-end gap-3 px-5 py-4 bg-zinc-50 dark:bg-zinc-950/50 border-t border-zinc-100 dark:border-zinc-800">
          ...
          <button
            ...
            disabled={loading || !colorName.trim()}
          >
            {loading ? <Loader2 size={15} className="animate-spin" /> : null}
            {loading ? (isEdit ? 'Saving…' : 'Creating…') : (isEdit ? 'Save' : 'Create color')}
          </button>
        </div>
      </div>
    </div>
